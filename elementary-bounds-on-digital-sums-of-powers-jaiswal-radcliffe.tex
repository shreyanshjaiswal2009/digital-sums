...
\author{David G. Radcliffe and Shreyansh Jaiswal} % Alphabetical order implemented
\date{December 13, 2025}
\keywords{Sum of digits, powers, factorials, least common multiples.}
\subjclass[2020]{Primary 11A63; Secondary 11A25.}
...
holds if no carries occur in the digit-wise addition of $M$ and $N$.
...
One writes $f(n) = O(g(n))$ if there exists a positive real number $C$ such that
\[
...
a function that tends to 0 as $n \to \infty$. Finally, $f(n) \asymp g(n)$ means that there exist positive real numbers $C$ and $C'$ such that
\[
...
Figure~\ref{fig-blocks}. %Note - In the PDF, this shows up as Figure 3, which is wrong. It should be Figure 2.
...
Subsequently, Stewart~\cite[Theorem 2]{stewart1980} (see~\cite{linearrecurrenceluca2000} for a slightly more general result.) proved
that if $\log(a)/\log(b)$ is irrational then
\[
	c_b(a^n) > \frac{\log n}{\log\log n + C} - 1
\]
for all $n>4$, where $C$ depends only on $a$ and $b$.
This bound is extremely general but grows more slowly than logarithmically.
...
digital sums of $n!$ and $\Lambda_n = \lcm(1,\ldots, n)$. We recall that $\Lambda_n$ has an important role in elementary proofs of the Chebyshev bounds $\pi(x) \asymp x/\log x$, for the prime counting function $\pi(x)$~\cite{mnairprimes1982}.
...
of $\alpha$ is the maximum of the absolute values of the coefficients of $P$.
...
\begin{thebibliography}{99}
    ...
    \bibitem{linearrecurrenceluca2000}
	Luca, F.,
    ...
    \bibitem{mnairprimes1982}
	Nair, M.,
	``On Chebyshev-Type Inequalities for Primes,''
	\textit{The American Mathematical Monthly} \textbf{89} (1982), pp. 126--129.
\end{thebibliography}
