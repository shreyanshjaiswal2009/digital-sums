...
\begin{document}
	...
	\section{Digital sums of powers: the general case}
		...
		This condition is necessary, because if $\log(a)/\log(b) = r/s \in \mathbb{Q}$, then
		$a^{ns} = b^{nr}$ for all $n$, so $a^{ns}$ has only one nonzero digit in its base-$b$
		expansion.

		The proof is not elementary, as it relies on a deep result in transcendental number theory,
		concerning linear forms in logarithms.
		...
		\begin{proof}
			Let
			\begin{equation}
				a^n = b^m (d_i b^{-m_1} + \cdots + d_r b^{-m_r}),
			\end{equation}
			where $m = \lfloor \log_b a \rfloor$, $r = c_b(a^n)$, $d_i \in \{1, \ldots, b-1\}$,
			and $1 = m_1 < \cdots < m_r \le m$. Assume that $r \ge 2$.

			Choose $i \in \{1, \ldots, r-1\}$. We seek an upper bound on $m_{i+1}/m_i$,
			which will imply an upper bound on $n$.

			Let $q = b^{m_i} (d_1 b^{-m_1} + \cdots + d_i b^{-m_i})$ and
			$r = b^m (d_{i+1} b^{-m_{i+1}} + \cdots d_r b^{-m_r})$, so that
			$a^n = b^{m - m_i} q + r$. 
			...
			\begin{align*}
			    b^{m-1} &< a^n < b^m, \\
                b^{m_i-1} &< q < b^{m_i}, \\
				b^{m-m_{i+1}} &< r < b^{m-m_{i+1}+1}.
			\end{align*}

			By combining these inequalities, we find that
			\begin{equation}
				-\log(a^{-n} r) / \log(q) \approx m_{i+1} / m_i.
			\end{equation}

			Let $\Lambda = n \log a + (m - m_i) \log b + \log q$.
			Then $|\Lambda| = \log(1 + a^{-n} r) < a^{-n} r$.
			But by Theorem \ref{baker}, $|\Lambda| > C^{-\log n \log q}$ for some $C > 1$
			depending only on $a$ and $b$. Therefore, $a^{-n} r > C^{-\log n \log q}$,
			so
			\begin{equation}
				-\log(a^{-n} r) / \log(q) < C' \log n.
			\end{equation}

			Since $\log m_r = \sum_{i=1}^{r-1} \log(m_{i+1}/m_i)$, Equations (2) and (3) imply that
			\[
			   m_r < r (\log \log n + C).
			\]

			But $m_r = \Theta(\log n)$ by Lemma \ref{lemma}, hence
			\[
