...
\begin{document}
	...
	\section{Generalizing to other bases}
		...
		\begin{proof}
			...
			so that $r^{k-1} < \xi(a^n) \le r^k$.
			Then $\xi(a^n) > \varepsilon(k)$, so Theorem \ref{phi-lower-bound} implies that $c_p(a^n) > k$.
			But $\xi(a^n) = n \xi(a)$, which implies that $k = \Theta(\log n)$, hence
