...
\begin{document}
	...
	\begin{abstract}
		...
		including factorials and the least common multiple of $\{1, \ldots, n\}$.
	...
	\section{Digital sums of powers of two}
		...
		\begin{proof}
			...
			Next, both $n$ and $10^{e(k-1)}$ are divisible by $2^{e(k-1)}$,
	...
	\section{Generalizing to other bases}
		...
		\begin{proof}
			...
			Assume now that $k \ge 2$, and that the statement holds for $k-1$.
			Let $n$ be a positive integer divisible by $p^{e(k)}$ but not by $b$.
			Apply the division algorithm to write
			\[
				n = b^{e(k-1)} q + r, \qquad 0 \le r < b^{e(k-1)},
			\]
			for integers $q$, $r$.

			Because $n \ge p^{e(k)} > b^{e(k-1)}$ by hypothesis,
			the quotient satisfies $q \ge 1$.

			Next, both $n$ and $b^{e(k-1)}$ are divisible by $p^{e(k-1)}$,
			hence their difference
			\[
				r = n - b^{e(k-1)} q
			\]
			is also divisible by $p^{e(k-1)}$.

			Moreover, $r$ is not divisible by $b$, since $n$ is not divisible by $b$.

			Therefore, $c_{b}(r) \ge k - 1$ by the induction hypothesis.

			Finally, the base-$b$ expansion of $n$ is obtained by concatenating
			the base-$b$ expansion of $q$ with the (possibly zero-padded)
			expansion of $r$. Thus,
			\[
		...
		In 2025, S. Jaiswal (personal communication) proved that if $a \ge 2$ and $b \ge 2$ are positive integers,
	...
	\section{Digital sums of powers: the general case}
		...
		\begin{proof}
			...
			\[
				\nu_p(a) \nu_q(b) > \nu_q(a) \nu_p(b).
			\]
			Hence $\xi(a)>0$ for the function $\xi=\xi_{b,p,q}$ defined in Equation \ref{xi-notation}.
